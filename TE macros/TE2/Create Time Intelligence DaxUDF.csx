...
// - Helper: is a TimeUnit a weekly unit? ----------------─
// TE2 (C# 5/6) does not support "is X or Y" pattern matching, so use a helper method.
Func<TimeUnit, bool> IsWeekUnit = tu =>
    tu == TimeUnit.Week ||
    tu == TimeUnit.WeekOfMonth ||
    tu == TimeUnit.WeekOfYear ||
    tu == TimeUnit.WeekOfQuarter;

Func<TimeUnit, bool> IsMonthUnit = tu =>
    tu == TimeUnit.Month ||
    tu == TimeUnit.MonthOfQuarter ||
    tu == TimeUnit.MonthOfYear;

Func<TimeUnit, bool> IsQuarterUnit = tu =>
    tu == TimeUnit.Quarter ||
    tu == TimeUnit.QuarterOfYear;
...
Action CheckCalc = () => {
    ...
    bool isWeekly  = cal != null && cal.GetTimeUnits().Any(t => IsWeekUnit(t.TimeUnit));
    bool hasMonth  = cal == null || cal.GetTimeUnits().Any(t => IsMonthUnit(t.TimeUnit));
    bool hasQuarter= cal == null || cal.GetTimeUnits().Any(t => IsQuarterUnit(t.TimeUnit));
...
// ===================== Resolve selections =====================
var selItem2     = dateColumnSelector.SelectedItem as ComboBoxItem;
var dateColumn   = selItem2.Column == null
                     ? selItem2.Calendar.DaxObjectFullName
                     : selItem2.Column.DaxObjectFullName;

var actualDateCol = (selItem2.Column != null)
                      ? selItem2.Column.DaxObjectFullName
...
class ComboBoxItem
{
    ...
    public ComboBoxItem(Column   column)   { _column   = column;   }
    public ComboBoxItem(Calendar calendar) { _calendar = calendar; }
    public override string ToString()
    {
        if (_calendar == null)
            return "Column: "   + _column.DaxObjectFullName;
        else
            return "Calendar: " + _calendar.DaxObjectFullName;
    }
}